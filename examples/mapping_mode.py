...
if __name__ == '__main__':
    ...
    main_utils.start_sending_heartbeat(connection, armed=True)
    ...
    try:
        ...
        old_state = mavlink.MAV_STATE_UNINIT

        print('Press Ctrl-C to terminate receiving vision position messages.')
        try:
            while True:
                msg = connection.recv_match(type=['HEARTBEAT', 'VISION_POSITION_ESTIMATE', 'STATUSTEXT'], blocking=True)
                ...
                elif msg.get_type() == 'VISION_POSITION_ESTIMATE':
                    print('Vision position (usec,x,y,z,roll,pitch,yaw): %d, %.2f, %.2f, %.2f, %1.3f, %1.3f, %1.3f' %
                          (msg.usec, msg.x, msg.y, msg.z, msg.roll, msg.pitch, msg.yaw)
                          )
        ...
        main_utils.stop_sending_heartbeat(connection)
        main_utils.start_sending_heartbeat(connection, armed=False)
        print('Waiting for saving map...')
        time.sleep(5)
